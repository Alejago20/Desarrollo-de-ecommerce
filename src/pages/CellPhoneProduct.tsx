...
import { VariantProduct } from '../interfaces/product.interface';
...
interface Acc{
  //una llave dinamica key va cambiando de valor, con base va cambiando de color 
  [key: string]:{
    name:string;
    storages:string[];
  }
...
export const CellPhoneProduct = () => {
   //debemos que tener formatear el producto para poderlo renderizar
 ...
 const navigate = useNavigate();
  ...
  /*    Este condicional se ejecuta para verificar si el color actual de la variante ya 
     ha sido agregado al objeto acc que estamos construyendo. */

   /*   Si no existe, crea una nueva entrada para ese color: */
     if (!acc[color]) {
         acc[color]={
          name: color_name,
          storages:[],
  ...
  /*    Verifica si el valor de storage no está incluido en el arreglo storages 
     de ese color */
  ...
  //Obtener el primer color predeterminado sino se ha seleccionado ninguna
  //convierte las llaves de un objeto en un arreglo (array).
  const availableColors=Object.keys(colors);
  useEffect(()=>{
 if (!selectedColor && availableColors.length>0) {
  setSelectedColor(availableColors[0]);
 }
    //cada vez que esto cambie se va efectuar el efecto
  },[availableColors,selectedColor]);

  //Actualizar el alamcenamiento seleccionado cuando cambia el color 
  //debemos tener en cuenta los almacenamietos disponibles en ese color 
   useEffect(()=>{
    if (selectedColor && colors[selectedColor] && !selectedStorage) {
      //Obtiene el primer almacenamiento disponible 
   ...
   //Obtener Stock
   const isOutOftStok=selectedVariant?.stock===0;
...
if (isLoading) return <Loader/>
  ...
  return (
    ...
    //para mostrar pagina de producto recuerda tomar el parametro que le estoy mandando a la URL
    //ES DECIR EL PARAMETRO DE SLUG QUE ESTOY ENVIANDO EN EL ROUTER
    <>
      <div className="h-fit flex flex-col md:flex-row gap-16 mt-8">
        {/* GALERIA DER IMAGENES */}
        ...
        <div className="flex-1 space-y-5">
          ...
          <div className="flex gap-5 items-center">
            <span className="tracking-wide text-lg font-semilbold">
              {formatPrice(selectedVariant?.price || product.variants[0].price)}
            </span>

            <div className="relative">
              {isOutOftStok &&  <Tag contentTag='agotado'></Tag>}
              {/*      {TAG -> Agotado} */}
